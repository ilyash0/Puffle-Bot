...
class GameQuest(db.Model):
    ...
    id = db.Column(db.Integer(), nullable=False, primary_key=True, server_default=db.text("nextval('\"game_quest_id_seq\"'::regclass)"))
    type_id = db.Column(db.Integer(), nullable=False, server_default='0')
    coins = db.Column(db.Integer(), nullable=False, server_default='500')
    item = db.Column(db.Integer(), nullable=False, server_default='0')
    furniture = db.Column(db.Integer(), nullable=False, server_default='0')


class PenguinQuest(db.Model):
    __tablename__ = 'penguin_quest'

    id = db.Column(db.Integer(), nullable=False, primary_key=True, server_default=db.text("nextval('\"game_quest_id_seq\"'::regclass)"))
    penguin_id = db.Column(db.Integer(), nullable=False, server_default='0')
    quest_id = db.Column(db.Integer(), nullable=False, server_default='500')
    argv1 = db.Column(db.Integer(), nullable=False, server_default='0')
    argv2 = db.Column(db.Integer(), nullable=False, server_default='0')
    status = db.Column(db.Integer(), nullable=False, server_default='0')
    expires = db.Column(db.DateTime, nullable=False, server_default=db.text("(now() + '23:59:59'::interval)"))
